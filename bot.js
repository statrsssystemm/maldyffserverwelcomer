...
client.on('message', message => {
    if (message.content === '.') {
        message.reply('آحلي من نقط');
      }
});
    
    client.on('message', message => {
    if (message.content === '..') {
        message.reply('**نقطهه ولا عسل**');
      }
});
        
client.on('message', message => {
    if (message.content === '...') {
        message.reply('خلاص زودتها عاد');
      }
});
...
client.on("message", (message) => {
    if (message.content.startsWith("صوتي")) {
...
//////////////////////////////////////////
client.on("message", (message) => {
    if (message.content.startsWith("كتابي")) {
                if (!message.member.hasPermission('MANAGE_CHANNELS')) return message.reply("You Don't Have `MANAGE_CHANNELS` Premissions ");
            let args = message.content.split(" ").slice(1);
...
Client.on("message", message => {
	...
	if( msg.toLowerCase().startsWith( prefix + 'سحب الصلاحية' ) ){
		...
		} else if( args[0].toLowerCase() == "بوتات" ){
			message.guild.members.filter(m=>m.user.bot).forEach(m=>m.removeRole(role1))
			return	message.reply('**:white_check_mark: [ '+role1.name+' ] تم سحب من البوتات رتبة**');
		} else if( args[0].toLowerCase() == "اعضاء" ){
			message.guild.members.filter(m=>!m.user.bot).forEach(m=>m.removeRole(role1))
...
client.on('message', message => {
    ...
    if(message.content.includes('.com')){
      if(!message.member.hasPermission('MANAGE_MESSAGE'))
        message.delete()
    return message.reply(`**يمنع إرسال الروابط هنا ×**`)
    }
});
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
client.on('message', message => {
